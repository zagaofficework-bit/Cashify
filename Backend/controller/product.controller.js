  ...
  try {
    ...
    if (req.body.specs) {
      try {
        ...
        return res.status(400).json({ message: "Invalid specs format — must be valid JSON" });
      }
    }
    ...
    const productData = {
      ...
      status: "available",
      ...(specs && { specs }),  // attach specs if provided
    };
...
exports.upsertSpecs = async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(403).json({ message: "Not authorized to update specs for this product" });
    }

    const { performance, display, rearCamera, frontCamera, battery, storageType } = req.body;

    // Build only the specs fields that were sent
    const specsUpdate = {};
    if (performance  !== undefined) specsUpdate["specs.performance"]  = performance;
    if (display      !== undefined) specsUpdate["specs.display"]      = display;
    if (rearCamera   !== undefined) specsUpdate["specs.rearCamera"]   = rearCamera;
    if (frontCamera  !== undefined) specsUpdate["specs.frontCamera"]  = frontCamera;
    if (battery      !== undefined) specsUpdate["specs.battery"]      = battery;
    if (storageType  !== undefined) specsUpdate["specs.storageType"]  = storageType;

    if (Object.keys(specsUpdate).length === 0) {
      return res.status(400).json({ message: "No spec fields provided to update" });
    }

    const updated = await ProductService.updateSpecs(req.params.id, specsUpdate);
...
exports.compareProducts = async (req, res) => {
  try {
    ...
    if (idList.length < 2) {
      return res.status(400).json({ message: "Provide at least 2 product IDs to compare" });
    }
    if (idList.length > 4) {
      return res.status(400).json({ message: "Cannot compare more than 4 products" });
    }

    const invalid = idList.filter((id) => !isValidObjectId(id));
    if (invalid.length > 0) {
      return res.status(400).json({ message: `Invalid product ID(s): ${invalid.join(", ")}` });
    }
    ...
    if (notFound.length > 0) {
      return res.status(404).json({ message: `Product(s) not found: ${notFound.join(", ")}` });
    }
    ...
    const comparison = products.map((p) => ({
      ...
      display: p.specs?.display
        ? {
            size:        p.specs.display.sizeInches && p.specs.display.sizeCm
              ? `${p.specs.display.sizeInches} (${p.specs.display.sizeCm})`
              : p.specs.display.sizeInches || "N/A",
            type:        p.specs.display.type        || "N/A",
            resolution:  p.specs.display.resolution && p.specs.display.resolutionType
