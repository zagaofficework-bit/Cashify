...
//// Frontend uses this to build the "add variant" form dynamically
////////////////////////////////////////////////////////////////////
...
exports.getVariantsByModel = async (req, res) => {
  try {
    ...
    res.status(200).json({
      ...
      specFields: fields, // tells the frontend what labels to show
...
function computePricing({ config, answers, defectKeys, accessoryKeys, basePrice }) {
  ...
  // ── Defects (flat key list — search children first, then top-level) ──
  if (defectKeys?.length > 0) {
    for (const key of defectKeys) {
      let found = false;
 
      // Search in children of each defect group
      ...
      // Fallback: top-level defect with no children
      if (!found) {
...
exports.submitListing = async (req, res) => {
  try {
    ...
    const listing = await DeviceListing.create({
      ...
      model:     model.name,
      image:     model.image,
      specs:     variant.specs, // ✅ flexible specs map
...
exports.completeListing = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, message: "Transaction completed successfully", data: { listingId: listing._id, finalPrice: listing.finalPrice, completedAt: listing.completedAt } });
  } catch (error) {
...
exports.rejectListing = async (req, res) => {
  try {
    ...
    if (seller.isSuperSeller) {
      ...
      await listing.save();
      return res.status(200).json({ success: true, message: "Listing rejected. It is now visible to all regular sellers.", data: { listingId: listing._id, visibility: "all_sellers", status: "available", reason: reason?.trim() || null } });
    }
    ...
    res.status(200).json({ success: true, message: "Listing rejected. It is back in the open pool for other sellers.", data: { listingId: listing._id, visibility: listing.visibility, status: "available", reason: reason?.trim() || null } });
  } catch (error) {
...
exports.dismissListing = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, message: "Listing dismissed. It is now visible to all regular sellers.", data: { listingId: listing._id, visibility: "all_sellers", status: "available" } });
  } catch (error) {
...
exports.addBrandCatalog = async (req, res) => {
  try {
    ...
    // Validate variant specs against the category's field map
    const fields = VARIANT_FIELDS_MAP[category];
...
exports.updateEvaluationConfig = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, message: `Evaluation config updated for category: ${category}`, data: config });
  } catch (error) {
