...
const CashifyLogo = () => (
  <svg width="18" height="18" viewBox="0 0 40 40" fill="none">
    ...
    <path
      d="M20 8C13.373 8 8 13.373 8 20s5.373 12 12 12 12-5.373 12-12S26.627 8 20 8zm0 3a9 9 0 110 18A9 9 0 0120 11zm-1 4v2.07A4.003 4.003 0 0020 25a4 4 0 001-7.93V15h-2zm1 4a2 2 0 110 4 2 2 0 010-4z"
      fill="white"
    />
  </svg>
);
...
const AssuredShield = () => (
  <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
    ...
    />
    <path
      d="M10 14.4l-2.4-2.4-1.2 1.2 3.6 3.6 7.2-7.2-1.2-1.2L10 14.4z"
      fill="white"
    />
  </svg>
);
  ...
  return (
    <div className="flex-shrink-0 w-[230px] bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 cursor-pointer flex flex-col group">
      <div className="relative bg-white px-6 pt-5 pb-3 h-[200px] flex items-center justify-center">
        {savedAmount && (
          <div className="absolute top-0 left-0 z-10">
            <div
              className="bg-[#1a6b3c] text-white text-[11px] font-bold px-3 py-1.5 rounded-br-xl rounded-tl-2xl leading-tight"
      ...
      <div className="px-4 pt-3 pb-4 flex flex-col gap-0 flex-grow">
        <h3
          className="text-[13.5px] font-bold text-gray-900 leading-snug line-clamp-2"
        ...
        <div className="flex items-center gap-2 mt-2">
          {product.saleTag && (
            <span className="bg-yellow-50 text-yellow-700 border border-yellow-200 text-[10.5px] font-semibold px-2 py-0.5 rounded-md">
              {product.saleTag}
            </span>
          )}
          {product.rating > 0 && (
        ...
        <p
          className="text-[22px] font-extrabold text-gray-900 leading-tight mt-0.5"
          style={{
        ...
        <div className="flex items-center gap-1.5 mt-1 flex-wrap">
          {product?.specs?.performance?.ram && (
            <span className="text-xs font-semibold bg-blue-50 text-blue-600 border border-blue-200 rounded px-1.5 py-0.5 leading-tight">
              {product.specs.performance.ram} RAM
            </span>
          )}
          {product?.storage && (
            <span className="text-xs font-semibold bg-gray-100 text-gray-600 border border-gray-200 rounded px-1.5 py-0.5 leading-tight">
              {product.storage}
            </span>
          )}
          {product?.specs?.battery?.capacity && (
            <span className="text-xs font-semibold bg-green-50 text-green-600 border border-green-200 rounded px-1.5 py-0.5 leading-tight">
              {product.specs.battery.capacity}
            </span>
          )}
          {product?.specs?.frontCamera && (
            <span className="text-xs font-semibold bg-purple-50 text-purple-600 border border-purple-200 rounded px-1.5 py-0.5 leading-tight">
              {product.specs.frontCamera} Front
            </span>
          )}
        </div>
        ...
        <div className="mt-3 flex items-center gap-1.5 bg-[#fdf6e3] border border-[#f0d98a] rounded-xl px-3 py-2">
          <AssuredShield />
          <span className="text-[12.5px] font-bold text-[#b07d0d]">
            Cashify Assured
          </span>
        </div>
        ...
        <div className="mt-2.5 flex items-center gap-1.5">
          ...
          <div className="flex items-center gap-1">
            <CashifyLogo />
            <span
              className="text-[12px] font-extrabold text-[#00a86b]"
...
export default function BuyRefurbishedDevices({ title }) {
  ...
  const handleviewall = () => {
    // Implement view all functionality, e.g., navigate to a full listing page
    ...
    <section className="py-12 bg-[#f5f5f5]">
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex items-center justify-between mb-7">
          ...
          <p
            className="text-sm font-semibold text-[#00a86b] hover:underline flex items-center gap-1 cursor-pointer"
        ...
        <div className="relative px-5">
          ...
          {!loading && !error && products.length === 0 && (
            <p className="text-gray-500 text-sm">
              No refurbished mobiles available right now.
            </p>
          )}
          {!loading && !error && products.length > 0 && (
            <>
