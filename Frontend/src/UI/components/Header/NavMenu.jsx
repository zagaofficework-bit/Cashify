  ...
  const menuItems = [
    {
      title: "Brands",
      subItems: [
        { label: "Apple", path: "/sell-apple" },
        { label: "OnePlus", path: "/sell-oneplus" },
        { label: "Samsung", path: "/sell-samsung" },
        { label: "Xiaomi", path: "/sell-xiaomi" },
      ],
    },
    {
      title: "Sell Gadgets",
                ...
                <div className="bg-gray-50 pb-3">
                  ...
                  {item.subItems.map((sub, si) => (
                    <div key={si} className="mt-1">
                      ...
                      {/* ✅ FIXED: was {s} (object), now {s.label} with navigate */}
                      {sub.subs?.map((s, ti) => (
                        <div
                          key={ti}
                          onClick={() => s.path && navigate(s.path)}
                          className="flex items-center gap-2 px-8 py-1.5 text-xs text-gray-500 hover:text-teal-600 cursor-pointer transition-colors"
                        >
